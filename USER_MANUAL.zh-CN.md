...
2. 已安装 `[search]` 扩展依赖。
3. 已配置嵌入密钥或兼容端点。
4. 嵌入端点能够为当前 `embed_model` 构建向量索引。
...
导入成功后，应用只会索引本次导入触及的概念页。在执行语义 Search 或 Ask 前，应用会让
LanceDB 与请求范围及其后代中的概念 Markdown 保持一致：嵌入缺失或已变化的片段，并移除
已删除页面，也会处理通过 Git 或其他 worktree 到达的变化。页面未变化时，查询仍会进行
正常的查询嵌入，但不会重新嵌入页面内容。

文件系统变化后的第一次语义查询可能更慢，并会把该范围内已变化的概念片段发送到配置的
嵌入端点。设置 `hybrid_search = false` 可禁用同步。同步失败时，检索会在日志中记录一次
警告并返回 BM25 结果。来源页仍可能通过本地标题/slug 匹配出现。如果缺少预期结果，先
扩大当前范围。

可运行 `llmwiki reindex`，在第一次查询前构建整个知识库索引或显式修复索引。同一知识库
请同时只运行一个写入向量的 llmwiki 进程。协调仅在进程内生效，因此 Web 与 CLI 同时写入
可能重复嵌入工作，或短暂留下过期的派生行；下一次语义查询会进行校准。
...
更改嵌入模型或安装搜索扩展依赖后，下一次语义操作会自动构建匹配的索引。可运行
`llmwiki reindex` 提前构建，或运行 `llmwiki reindex --force` 显式重新嵌入未变化页面。
启用 rerank 会增加延迟，并让每次 Ask 多一次模型调用；失败时会回退到原始检索顺序。
