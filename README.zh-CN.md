...
从仓库检出目录安装搜索扩展依赖，并配置嵌入模型密钥或兼容 OpenAI 的端点：

```bash
python -m pip install ".[web,search]"
```

导入成功后，应用只会索引本次导入触及的概念页。在执行语义 Search 或 Ask 前，应用会对
请求范围及其后代进行校准：嵌入缺失或已变化的片段，并移除已删除页面。这样，通过 Git 或
其他 worktree 到达的 Markdown 变化会自动修复，而不会发送无关范围。若要提前构建整个
知识库的索引，可运行 `llmwiki reindex`。

文件系统变化后的第一次语义查询可能更慢，并会把该范围内已变化的概念片段发送到配置的
嵌入端点。设置 `hybrid_search = false` 可禁用此行为。缺少扩展依赖或嵌入端点时，Search
会回退到本地 BM25 排序。同一知识库请同时只运行一个写入向量的 llmwiki 进程；协调仅在
进程内生效。
